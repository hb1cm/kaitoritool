...
with tab_upload:
    ...
    # --- ✨ 新增：操作指南 ---
    with st.expander("📖 Cross Mall からのデータダウンロード手順（必ずお読みください）", expanded=True):
        st.markdown("""
        Cross Mallから以下の設定でデータを書き出してアップロードしてください。
        ...
        ---
        **⚠️ 注意事項**
        - **重複注意：** データベースの容量を節約するため、アップロード済みのデータを再度アップロードしないでください。
        - **加工禁止：** ダウンロードしたCSVは**開かずに（編集せずに）**そのままアップロードしてください。
        """)
